...
// binary's own `update` command. A binary that cannot be found gets no update
// command, the same answer the package-managed path gives.
...
export function codexMaintenanceResolver(
  ...
  return updateSource === "lhc" ? LHC_UPDATE : makeCodexMaintenanceResolver(sharedHomePath);
}
  ...
  create: ({ instanceId, displayName, accentColor, environment, enabled, config }) =>
    Effect.gen(function* () {
      ...
      const resolveMaintenance = yield* makeCachedProviderMaintenanceResolution(
        resolveProviderMaintenanceCapabilitiesEffect(
          codexMaintenanceResolver(effectiveConfig.updateSource, homeLayout.sharedHomePath),
          {
