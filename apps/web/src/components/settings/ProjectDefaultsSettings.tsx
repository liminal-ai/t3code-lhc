    ...
    >
      {category === "general" ? (
        <>
          ...
          <SettingsRow
            serverScoped
            settingKeys={["hideFullAccess"]}
            {...searchableSetting("offer-full-access")}
            title="Offer Full access"
            description="When hidden, the composer does not list Full access. Existing threads keep their mode. Cannot be hidden while the default permissions are Full access."
            resetAction={
              settings.hideFullAccess !== DEFAULT_SERVER_SETTINGS.hideFullAccess ? (
                <SettingResetButton
                  label="offer Full access"
                  onClick={() =>
                    updateSettings({ hideFullAccess: DEFAULT_SERVER_SETTINGS.hideFullAccess })
                  }
                />
              ) : null
            }
            control={
              <Select
                value={settings.hideFullAccess ? "hidden" : "offered"}
                onValueChange={(value) => {
                  if (value === "offered" || value === "hidden")
                    updateSettings({ hideFullAccess: value === "hidden" });
                }}
              >
                <SelectTrigger size="sm" aria-label="Offer Full access">
                  <SelectValue>{settings.hideFullAccess ? "Hidden" : "Offered"}</SelectValue>
                </SelectTrigger>
                <SelectPopup align="end" alignItemWithTrigger={false}>
